...
// Removed duplicate compareBtn listener
...
function renderGrid() {
    ...
    allCheckIns.forEach(checkin => {
        ...
        card.innerHTML = `
            ...
            ${isSelected ? '<div style="position:absolute; top:8px; right:8px; background:var(--primary); color:white; width:24px; height:24px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.3); border: 2px solid white; z-index:10;"><i class="fas fa-check"></i></div>' : ''}
        `;
...
function updateComparisonBar() {
    ...
    if (selectedCheckIns.length > 0) {
        ...
        comparisonBar.style.display = 'flex'; // Force show

        // Show delete button only if 1 item is selected (as a quick action)
        if (selectedCheckIns.length === 1) {
        ...
        comparisonBar.style.display = 'none'; // Force hide
    }

    compareBtn.disabled = selectedCheckIns.length < 2;
    compareBtn.style.opacity = selectedCheckIns.length >= 2 ? '1' : '0.5';
    compareBtn.textContent = selectedCheckIns.length >= 2 ? (selectedCheckIns.length > 2 ? "Analyze Journey" : "Compare with AI") : "Select 2+ to Compare";
}

if (deleteSelectedBtn) {
    deleteSelectedBtn.onclick = () => {
        if (selectedCheckIns.length === 1) {
            const id = selectedCheckIns[0].id;
            deleteCheckin(id);
        }
    };
}
